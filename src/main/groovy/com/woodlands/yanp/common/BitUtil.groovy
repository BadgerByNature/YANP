...
*/
package com.woodlands.yanp.common
...
class BitUtil {
     ...
     */
    static final byte[] readLECString(ByteBuf buf) {
     ...
     */
    static final byte[] readLECString(ByteBuf buf, int length) {
        Objects.requireNonNull(buf, "buffer cannot be null")
        buf.markReaderIndex()
     ...
     */
    static final void readLE(ByteBuf buf, byte[] destBuf) {
     ...
     */
    static final void reverseBuffer(byte[] arr) {
     ...
     * @param arr The byte array to reverse
     */
    static final void reverseBuffer(byte[] arr, int length) {
     ...
     */
    static final byte[] reverse(byte[] arr) {
     ...
     */
    static final BigInteger toBigInteger(byte[] buf, int length, boolean littleEndian) {
     ...
     * @return The {@link BigInteger} created from the byte array
     */
    static final BigInteger toBigInteger(byte[] arr, boolean littleEndian) {
        if (!littleEndian) {
            return new BigInteger(1, arr) // big-endian byte order
        }
